 ...
 * @author MXGC
 */
@WebServlet(name = "UpdateStatusOrderServlet", urlPatterns = {"/Admin/UpdateStatusOrderServlet"})
public class UpdateStatusOrderServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            String[] arry= {"Shipping","Completed","Processing"};
             int id = Integer.parseInt(request.getParameter("id"));
           
            Orders od =  ordersFacade.find(id);
           ordersFacade.edit(od);
           
           request.setAttribute("status", arry);
            request.setAttribute("st", od);
