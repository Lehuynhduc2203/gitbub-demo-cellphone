    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            for (int i = 0; i < listItem.size(); i++) {
                listItem.get(i).setQuantity(Integer.parseInt(quantity[i]));
                listItem.get(i).setTotalPrice(Integer.parseInt(quantity[i]) * listItem.get(i).getPrice());

            }
            orders.setSubTotal(0);
            for (OrdersDetails item : listItem) {
                orders.setSubTotal(orders.getSubTotal() + item.getTotalPrice());
            }
